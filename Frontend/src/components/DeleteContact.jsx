...
const DeleteContact = ({ contactList, setDeleteContact, setContacts, setSelected }) => {
  const handleDelete = async () => {
    for (var id of contactList) {
      ...
      setContacts(contacts => contacts.filter(c => c._id !== id._id));
    }
    setDeleteContact(null)
    if (setSelected != null) {
      setSelected([])
    }
    toast.success(`${contactList.length} Contacts Deleted!`);
  };
  ...
  return (
    <section className='fixed top-0 left-0 w-screen h-screen flex items-center justify-center z-[1000]'>
      ...
      <aside className='relative'>
        <div className="flex flex-col items-center bg-white dark:bg-gray-800 shadow-md rounded-xl py-6 px-5  border border-gray-200">
          ...
          <div className='max-w-2xl flex flex-col overflow-x-auto mt-5'>
            ...
            <div className='flex gap-2 mt-10 justify-center mb-5'>
              {contactList.map((value, ind) => (
                <p key={ind} onClick={() => setIndex(ind)} className={`w-2 h-2 rounded-full cursor-pointer hover:scale-125 transition duration-150 ${ind == index ? "bg-blue-500" : "bg-gray-300"}`}></p>
              ))}
            </div>
          </div>
          <div className="flex items-center justify-center gap-4 mt-3 w-full">
            <button onClick={() => setDeleteContact(null)} type="button" className="cursor-pointer w-full md:w-36 h-10 rounded-md border border-gray-300 bg-white text-gray-600 font-medium text-sm hover:bg-gray-100 active:scale-95 transition">
              Cancel
            </button>
            <button onClick={handleDelete} type="button" className="cursor-pointer w-full md:w-36 h-10 rounded-md text-white bg-red-600 font-medium text-sm hover:bg-red-700 active:scale-95 transition">
              Confirm
            </button>
          </div>
        </div>
